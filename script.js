...
function openMediaViewer(src, type = 'image', title = '') {
  ...
  if (type === 'video') { window.open(mediaSrc, '_blank', 'noopener'); return false; }
  if (window.EpitesNaploUnifiedPhotoViewer?.openBySrc) return window.EpitesNaploUnifiedPhotoViewer.openBySrc(mediaSrc, title || 'Építési fotó');
  return false;
}
