// V142 – egységes képnéző: korai elfogó + régi képnéző végleges kikapcsolása
...
(function(){
  ...
  function hideOldViewers(){
    try{
      OLD_VIEWERS.forEach(sel => document.querySelectorAll(sel).forEach(el => {
        if(el && el.id !== 'v140UnifiedViewer') el.remove();
      }));
  ...
  function isReportOrTimelineImage(el){
    ...
    if(el.closest('#mediaViewerModal,#v67Lightbox,.v134Lightbox,.v121Lightbox,.v118Lightbox,.v110Gallery,.v103Lightbox,.v102Lightbox,.v100Lightbox,.v86Lightbox,.v79ReportLightbox,.v79GalleryModal,#v77Lightbox')) return false;
    ...
    return !!el.closest('.mediaTile,.mediaTileImage,.entryImageGrid,.detailImages,#projectTimeline,#publicReportContent,.v121Photos,.v134Photos,.v117Photos,.v67ReportPhoto,.v70PhotoBlock,.photos,.reportImageGrid,.reportMediaTile');
  }
  ...
  function ensureCss(){
    ...
    st.textContent = `
      #v140UnifiedViewer{position:fixed!important;inset:0!important;z-index:2147483640!important;background:rgba(2,6,23,.96)!important;display:none;color:#fff;font-family:Arial,Helvetica,sans-serif;touch-action:none}
      #v140UnifiedViewer.open{display:block!important}
      body.v140Open #mediaViewerModal,body.v140Open .mediaViewerModal,body.v140Open #v67Lightbox,body.v140Open .v67Lightbox,body.v140Open .v134Lightbox,body.v140Open .v121Lightbox,body.v140Open .v118Lightbox,body.v140Open .v110Gallery,body.v140Open .v103Lightbox,body.v140Open .v102Lightbox,body.v140Open .v100Lightbox,body.v140Open .v86Lightbox,body.v140Open .v79ReportLightbox,body.v140Open .v79GalleryModal,body.v140Open #v77Lightbox,body.v140Open .v77Lightbox{display:none!important;visibility:hidden!important;pointer-events:none!important}
      .v140Top{position:absolute;left:0;right:0;top:0;min-height:58px;background:#081225!important;color:#fff;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 12px;box-shadow:0 8px 22px rgba(0,0,0,.28);z-index:2147483642}
      ...
      .v140Stage{position:absolute;inset:58px 0 0 0;display:flex;align-items:center;justify-content:center;overflow:hidden;padding:22px 66px;touch-action:none;z-index:2147483641}.v140Stage img{max-width:100%;max-height:100%;object-fit:contain;border-radius:14px;background:#000;box-shadow:0 22px 70px rgba(0,0,0,.42);transform-origin:center center;will-change:transform;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}.v140Stage.full img{max-width:none;max-height:none;width:auto;height:auto}
      .v140Nav{position:absolute;top:50%;transform:translateY(-50%);width:46px;height:72px;font-size:36px;padding:0;z-index:2147483642;background:rgba(15,23,42,.86)!important}.v140Nav.prev{left:9px}.v140Nav.next{right:9px}
  ...
  document.addEventListener('click', function(e){
    let img = e.target && e.target.closest && e.target.closest(SEL);
    const expandBtn = e.target && e.target.closest && e.target.closest('.mediaExpandBtn,.v67ReportPhotoOpen,[onclick*=\"openMediaViewer\"],[onclick*=\"openMediaViewerFromTile\"],[onclick*=\"openReportMediaLink\"]');
    if(expandBtn && !img){
      const tile = expandBtn.closest('.mediaTile,.reportMediaTile,.v121Photo,.v134Photo,.v67ReportPhoto,figure,.photos,.v70PhotoBlock');
      img = tile && tile.querySelector && tile.querySelector('img');
    }
  ...
  // A régi scriptek egy része később újra felülírná a függvényeket. Ezért párszor újra rögzítjük az új nézőt.
  function installOverrides(){
    window.openMediaViewer = function(src, type, title){
      ...
      if(existing) openForElement(existing);
      else {
        ensureCss();
        const v = viewer();
        index = 0; scale = 1; tx = 0; ty = 0;
        document.getElementById('v140Title').textContent = (title || 'Napló fotó') + ' (1/1)';
        document.getElementById('v140Stage').innerHTML = '<img src="'+esc(src || '')+'" alt="'+esc(title || 'Napló fotó')+'">';
        v.classList.add('open');
        document.body.classList.add('v140Open');
      }
      return false;
    ...
    window.openMediaViewerFromTile = function(button, type, title){
      const tile = button && button.closest && button.closest('.mediaTile,.reportMediaTile,figure,.v121Photo,.v134Photo,.v67ReportPhoto,.photos,.v70PhotoBlock');
      const img = tile && tile.querySelector && tile.querySelector('img');
      if(img){ hideOldViewers(); openForElement(img); }
      return false;
    };
    window.openReportMediaLink = function(event, link){
      if(event){ event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation && event.stopImmediatePropagation(); }
      const tile = link && link.closest && link.closest('.reportMediaTile,.mediaTile,figure,.v121Photo,.v134Photo,.v67ReportPhoto,.photos,.v70PhotoBlock');
      const img = tile && tile.querySelector && tile.querySelector('img');
      if(img){ hideOldViewers(); openForElement(img); }
      return false;
    };
    window.closeMediaViewer = close;
    window.EpitesNaploOpenPhotoV140 = function(src, title){ return window.openMediaViewer(src, 'image', title || 'Napló fotó'); };
    window.EpitesNaploOpenPhotoV141 = window.EpitesNaploOpenPhotoV140;
    window.EpitesNaploOpenPhotoV142 = window.EpitesNaploOpenPhotoV140;
  }
  installOverrides();
  [0,50,250,700,1500,3000].forEach(ms => setTimeout(installOverrides, ms));
  try{
    new MutationObserver(() => {
      if(document.body.classList.contains('v140Open')) hideOldViewers();
