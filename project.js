...
// ===== V96 FINAL: működő v92 alap + vendég infó + ügyfél kérdés riportban + anyag duplázás nélkül =====
(function(){
  if (window.__epitesNaploV96FinalSafeFix) return;
  window.__epitesNaploV96FinalSafeFix = true;
  ...
  function normalizeMaterial(raw){
    ...
    const name = String(raw.name || raw.material || raw.title || raw.megnevezes || '').trim();
    const unit = String(raw.unit || raw.unit_name || raw.mertekegyseg || 'db').trim();
    const quantity = Number(String(raw.quantity ?? raw.qty ?? raw.amount ?? raw.mennyiseg ?? '').replace(',', '.'));
    if(!name || !Number.isFinite(quantity) || quantity <= 0) return null;
    return { name, unit, quantity };
  }

  function materialArrayFromEntry(entry){
    const src = entry?.materials_json ?? entry?.materials ?? entry?.material_json;
  ...
  function collectStructuredMaterials(entries, fallbackMaterials){
    const map = new Map();
    let foundEntryMaterials = false;
    (Array.isArray(entries) ? entries : []).forEach(entry => {
      const list = materialArrayFromEntry(entry);
      if(list.length) foundEntryMaterials = true;
      list.forEach(raw => {
        const m = normalizeMaterial(raw);
        if(!m) return;
        const key = `${m.name.toLowerCase()}|${m.unit.toLowerCase()}`;
    ...
    // Csak akkor használunk projekt szintű anyaglistát, ha a bejegyzésekben nincs strukturált anyag.
    // Így nem lesz 8 m³-ből 16 m³ a riportban.
    if(!foundEntryMaterials){
      (Array.isArray(fallbackMaterials) ? fallbackMaterials : []).forEach(raw => {
        const m = normalizeMaterial(raw);
        if(!m) return;
        const key = `${m.name.toLowerCase()}|${m.unit.toLowerCase()}`;
        const prev = map.get(key) || { name:m.name, unit:m.unit, quantity:0 };
  ...
  function materialSummaryHtml(entries, options){
    const list = collectStructuredMaterials(entries, options?.materials || options?.projectMaterials || []);
    return list.length
      ? `<ul>${list.map(m => `<li><b>${esc(m.name)}</b>: ${esc(String(m.quantity).replace('.', ','))} ${esc(m.unit)}</li>`).join('')}</ul>`
      : '<ul><li>Nincs rögzített anyag.</li></ul>';
  }

  function fixMaterialSummary(html, entries, options){
    let out = String(html || '');
    const replacement = `$1${materialSummaryHtml(entries, options)}`;
    if(/(<h2[^>]*>\s*Anyagösszesítő\s*<\/h2>\s*)<ul>[\s\S]*?<\/ul>/i.test(out)){
      return out.replace(/(<h2[^>]*>\s*Anyagösszesítő\s*<\/h2>\s*)<ul>[\s\S]*?<\/ul>/i, replacement);
    }
    return out;
  }

  function clientQuestionBlock(row){
    const msg = rowMessage(row);
    if(!msg) return '';
    return `<section class="v96ClientQuestion" style="margin:22px 0;padding:16px 18px;border-left:5px solid #f59e0b;background:#fff7ed;border-radius:12px;color:#111827;break-inside:avoid;page-break-inside:avoid;"><h2 style="margin:0 0 8px;">Ügyfél kérdése / észrevétele</h2><p style="white-space:pre-wrap;margin:0;">${esc(msg)}</p></section>`;
  }

  function injectClientQuestion(html, row){
    const block = clientQuestionBlock(row);
    if(!block) return html;
    let out = String(html || '');
    if(out.includes('v96ClientQuestion') || out.includes('v89ClientQuestion')) return out;
    if(/<h2[^>]*>\s*Anyagösszesítő\s*<\/h2>/i.test(out)){
      return out.replace(/(<h2[^>]*>\s*Anyagösszesítő\s*<\/h2>)/i, `${block}$1`);
    }
    if(/<body[^>]*>/i.test(out)) return out.replace(/<body[^>]*>/i, m => `${m}${block}`);
    return block + out;
  }

  function isBadSnapshot(html){
    const text = String(html || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    return !text.trim() || text.length < 500 || text.includes('jovahagyott riport tartalma nem talalhato') || text.includes('de a jovahagyas rekordja elerheto') || text.includes('hianyzo riport azonosito') || text.includes('riport adatai nem tolthetok');
  }

  function wrapReportHtml(html, row){
    let out = String(html || '');
    ...
    const msg = rowMessage(row);
    const stamp = `<section class="v96ApprovalStamp" style="border:2px solid #22c55e;background:#ecfdf5;color:#111827;padding:18px 20px;margin:0 0 22px;border-radius:14px;break-inside:avoid;page-break-inside:avoid;"><b>Jóváhagyott példány</b><br>Állapot: ${esc(rowLabel(rowDecision(row)))}<br>Elfogadás dátuma: ${esc(rowDate(row))}<br>Ügyfél: ${esc(row?.client_name || row?.client_email || 'Nincs megadva')}${msg ? `<div style="margin-top:12px;background:#fff7ed;border:1px solid #fdba74;border-radius:10px;padding:12px;"><b>Ügyfél kérdése / észrevétele:</b><br>${esc(msg).replace(/\n/g,'<br>')}</div>` : ''}</section>`;
    const css = `<style>
      @media(max-width:720px){body{padding:14px!important;overflow-x:hidden!important}.doc,.publicReportCard{max-width:100%!important;width:100%!important;padding:16px!important}h1{font-size:30px!important;line-height:1.08!important}h2{font-size:23px!important}table{display:block!important;width:100%!important;overflow-x:auto!important;white-space:nowrap!important}.stats{grid-template-columns:repeat(2,minmax(0,1fr))!important}.photos,.v74Photos,.v77Photos,.entryImageGrid,.reportImageGrid{grid-template-columns:repeat(auto-fill,minmax(92px,1fr))!important}.photo,.v67ReportPhoto,.v74Photo{width:92px!important;height:92px!important}.photo img,.v67ReportPhoto img,.v74Photo img{width:100%!important;height:100%!important;object-fit:cover!important}}
      @media print{.v96ApprovalStamp,.v96ClientQuestion{break-inside:avoid!important;page-break-inside:avoid!important}}
    </style>`;
    out = injectClientQuestion(out, row);
    out = out.includes('</head>') ? out.replace('</head>', `${css}</head>`) : `${css}${out}`;
    out = out.replace(/<body[^>]*>/i, m => `${m}${stamp}`);
    return out;
  }
  ...
  async function buildCurrentReportHtml(){
    const data = await window.EpitesNaploAPI?.getProjectCloseData?.(projectId()).catch(() => null);
    const entries = data?.entries || state()?.entries || [];
    const builder = window.buildProReportHtml || (typeof buildProReportHtml === 'function' ? buildProReportHtml : null);
    const html = builder ? builder(entries, `${projectTitle()} – jóváhagyott riport`, data || {}) : `<!doctype html><html lang="hu"><head><meta charset="utf-8"><title>${esc(projectTitle())}</title></head><body><h1>${esc(projectTitle())}</h1></body></html>`;
    return fixMaterialSummary(html, entries, data || {});
  }
  ...
  async function fullApprovedHtml(id){
    ...
    if(isBadSnapshot(html)) html = await buildCurrentReportHtml();
    try {
      const data = await window.EpitesNaploAPI?.getProjectCloseData?.(projectId()).catch(() => null);
      html = fixMaterialSummary(html, data?.entries || state()?.entries || [], data || {});
    } catch(_) {
      html = fixMaterialSummary(html, state()?.entries || [], {});
    }
    return { row, html: wrapReportHtml(html, row) };
  }
  ...
  window.v71DownloadApprovedHtml = async function(id){
    try {
      const r = await fullApprovedHtml(id);
      downloadHtml(`${safeName(projectTitle())}-${safeName(rowLabel(rowDecision(r.row)))}-jovahagyott-riport.html`, r.html);
    } catch(e) { alert(e.message || e); }
  };
